...
fn compare_table_matrix_enc() {
    ...
    for r in rec.records() {
        ...
        let alleles = sit.get_site_by_position(pos).unwrap();
        ...
        println!(
            "sit2: {pos}, {}",
            sit2.get_site_by_position(pos).unwrap().as_bstr()
        );
...
fn calc_xirs() {
    ...
    let xirs = XirsBuilder::new(afreq, gw_pos, &ibd)
        .unwrap()
        .finish()
        .unwrap();

    xirs.into_parquet("tmp.xirs.pq").unwrap();
