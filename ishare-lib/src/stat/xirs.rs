...
impl<'a> XirsBuilder<'a> {
    ...
    fn seg_2_x(&self, seg: &IbdSeg) -> Result<usize> {
    ...
    fn calculate_cm_xs(&mut self) -> Result<()> {
    ...
    fn calculate_raw(&mut self) -> Result<()> {
        let m = self.nsites;
        if self.rs_i.len() != m {
            self.calculate_rs();
        }
    ...
    // subtracted the mean and divided by the standard deviation of all values
    // within each bin (z-score)
    fn calculate_norm(&mut self) -> Result<()> {
        let m = self.nsites;
        // let n = self.npairs;
        if self.raw_i.len() != m {
            self.calculate_raw()?;
        ...
        annot_freq.sort_by(|a, b| a.1.total_cmp(&b.1));
    ...
    fn calculate_xirs(&mut self) -> Result<()> {
        let m = self.nsites;
    ...
    fn calculate_pval(&mut self) -> Result<()> {
        let m = self.nsites;
    ...
    pub fn finish(&mut self) -> Result<XirsResult> {
