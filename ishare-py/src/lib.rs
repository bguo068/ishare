...
impl Ibd {
    ...
    fn new(
        ...
        for (fname, chrname) in ibd_files.iter().zip(ginfo.chromnames.iter()) {
            if ibd_format == "hapibd" {
                ibdset.read_hapibd_file(fname, None).map_err(convert_err)?;
            } else if ibd_format == "tskibd" {
                ibdset
                    .read_tskibd_file(fname, chrname)
                    .map_err(convert_err)?;
            } else if ibd_format == "hmmibd" {
                ibdset.read_hmmibd_file(fname, None).map_err(convert_err)?;
            } else {
                return Err(PyErr::new::<PyValueError, _>(
    ...
    fn get_xirs<'py>(
        ...
        let xirs_res = match method {
            1 => XirsBuilder::new(afrq, site_pos, &ibdset)
                .map_err(convert_err)?
                .finish(),
            2 => XirsBuilder2::new(afrq, site_pos, &ibdset)
                .map_err(convert_err)?
                .finish(),
            _ => {
...
impl RVar {
    ...
    fn read_genotype(&mut self, prefix: String) -> PyResult<()> {
        ...
        self.records = Some(GenotypeRecords::from_parquet_file(gt_file).map_err(convert_err)?);
        Ok(())
    }
    ...
    fn read_sites(&mut self, prefix: String) -> PyResult<()> {
        ...
        self.sits = Some(Sites::from_parquet_file(sites_file).map_err(convert_err)?);
        Ok(())
    }
    ...
    fn read_inds(&mut self, prefix: String) -> PyResult<()> {
        let inds_file = from_prefix(&prefix, "ind")
            .map_err(|_e| PyErr::new::<PyValueError, _>("err in path to sites file"))?;
        self.inds = Some(Individuals::from_parquet_file(inds_file).map_err(convert_err)?);
        Ok(())
    }
    ...
    fn from_files(
        ...
        let records = if read_genotype {
            Some(GenotypeRecords::from_parquet_file(&gt_file).map_err(convert_err)?)
        } else {
            None
        };
        let sits = if read_sites {
            let sites = Sites::from_parquet_file(sites_file).map_err(convert_err)?;
            Some(sites)
        } else {
            None
        };
        let inds = if read_individuals {
            let inds = Individuals::from_parquet_file(ind_file).map_err(convert_err)?;
            Some(inds)
        } else {
            None
        };
