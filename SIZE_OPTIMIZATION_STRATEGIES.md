...
## Strategy 1: Box Large Context Fields ✅ Recommended
...
## Recommended Approach for ishare

### Phase 1: Box Context Fields (Low Risk, Good Benefit)

Apply to all errors with large context:
...
pub enum Error {
    ...
    ReadMapFile {
        source: std::io::Error,
        path: Box<PathBuf>,              // 8 bytes - saves 16 bytes
        backtrace: Option<Backtrace>,
    },
}
```
...
**Impact:**
- ~20% reduction in error size
- Preserves all backtraces
- Minimal code changes

### Phase 2: Consider Box<Error> for Hot Paths (Optional)
...
### Phase 3: Use Arc Where Errors Are Cloned (Optional)
...
// Optimized (64+ bytes) - 20% reduction
#[derive(Debug, Snafu)]
pub enum Error {
...
### Example 2: io.rs with Box<Error>
```rust
...
// Result<()> is now 8 bytes instead of 80+ bytes!
pub fn read_parquet(path: impl AsRef<Path>) -> Result<Data> {
    // Snafu automatically boxes the error
...
### Memory
- **Box<PathBuf>**: One extra heap allocation (16-24 bytes overhead)
- **Arc<PathBuf>**: One extra heap allocation + atomic ref count (24-32 bytes overhead)
- **Box<Error>**: One extra heap allocation (16 bytes overhead)

### CPU
- **Box/Arc construction**: Negligible (~1-2ns)
...
1. **Box<PathBuf> and other large contexts** - Simple, effective, preserves all debug info
   - 20% error size reduction
   - Minimal complexity increase
   - **Recommended for all large context fields**

2. **Box<Error> in Result** - For hot paths and tight loops
   - 90% Result size reduction
   - Only affects error path performance
   - **Recommended for frequently-called functions**

3. **Arc<PathBuf>** - When errors are cloned or stored
   - 20% error size reduction
   - Cheap cloning
   - **Recommended for errors that are propagated/stored**

**Avoid:**
- Removing backtraces (loses debug capability)
- Cow<str> (no size benefit over Box in practice)

**Bottom Line:**
You CAN significantly reduce error sizes (20-90%) while keeping full backtraces and context by using Box and Arc strategically. The extra allocations only occur on error paths, which are already slow, making this an excellent trade-off.
